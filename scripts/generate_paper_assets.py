...
x = ms/d
ax.plot(x, p_exact, color=COLORS['red'], lw=2, label='exact Beta--arcsine law')
ax.plot(x, 0.5 + np.sqrt(x)/np.pi, color=COLORS['blue'], lw=1.4, ls='--',
        label=r'asymptote $1/2+\pi^{-1}\sqrt{m/d}$')
mx, my = zip(*p_mc)
...
ax.set_title('Pairwise order preservation', fontsize=12)
ax.text(0, 1.005, 'Pairwise neighbour-order preservation, isotropic Gaussian, $d=200$',
...
ax.plot(ms/d, np.sqrt(np.minimum(ms/d, 1)), color=COLORS['blue'], lw=2,
        label=r'Haar projection $\sqrt{\min(m/d,1)}$')
ax.plot(ms/d, np.sqrt(ms/(ms+d+2)), color=COLORS['red'], lw=2,
        label=r'i.i.d. Gaussian $\sqrt{m/(m+d+2)}$')
mc = []
...
ax.set_title('Haar and Gaussian distance correlation', fontsize=12)
ax.text(0, 1.005, 'One-stage centered-distance correlation, $d=200$',
        transform=ax.transAxes, fontsize=8, color=COLORS['medium_gray'])
ax.legend(frameon=False, fontsize=8, loc='lower right')
polish(ax)
...
ms = np.arange(2, 59, 2)
fig, ax = plt.subplots(figsize=(5.2, 3.4))
ax.plot(ms/d, ms/d, color=COLORS['blue'], lw=2, label=r'mean direction / log scale:  $m/d$')
shape = (ms-1)*(ms+2)/((d-1)*(d+2))
ax.plot(ms/d, shape, color=COLORS['red'], lw=2,
        label=r'traceless shape:  $\frac{(m-1)(m+2)}{(d-1)(d+2)}$')
mc = []
...
mx, my = zip(*mc)
ax.scatter(np.array(mx)/d, my, color=COLORS['dark_gray'], s=22, zorder=5, label='Monte Carlo (traceless shape)')
ax.set_xlabel('$m/d$'); ax.set_ylabel('retained information fraction')
ax.set_title('Local information contraction', fontsize=12)
ax.text(0, 1.005, 'Local statistical contraction under Haar projection, $d=60$',
        transform=ax.transAxes, fontsize=8, color=COLORS['medium_gray'])
...
ax.plot(lam, [row['sqrt_alpha_1'] for row in rows], color=COLORS['dark_gray'],
        lw=1.6, ls='--', label=r'distance-value correlation ceiling $\sqrt{\alpha_1}$')
for degree, color in zip([2, 3, 4], [COLORS['blue'], COLORS['teal'], COLORS['red']]):
    ax.plot(lam, [row[f'cca_degree_{degree}'] for row in rows], color=color,
            lw=2, label=fr'polynomial degree $\leq {degree}$')
ax.axvline(2, color=COLORS['medium_gray'], lw=0.8, ls=':')
ax.set_xscale('log')
ax.set_xlim(1, 10)
ax.set_ylim(0.69, 1.005)
ax.set_xlabel(r'eigenvalue ratio  $\lambda_1/\lambda_2$')
ax.set_ylabel('canonical-correlation lower bound')
ax.set_title('Polynomial lower bounds under anisotropy', fontsize=12)
ax.text(0, 1.005, r'$D=\lambda G_1^2+G_2^2$, observed $U=\lambda G_1^2$; vertical line marks the exact counterexample',
        transform=ax.transAxes, fontsize=8, color=COLORS['medium_gray'])
ax.legend(frameon=False, fontsize=8, loc='lower right')
polish(ax)
